 ...
 * DBS_SimA.java
 * 
 * Model exported and heavily modified to be externally set 
 * and called by a GUI.
 * 
 */
...
public class DBS_SimA {
	...
	//0-7	: Xmax/Xmin,Ymax/Ymin,Zmax/Zmin,Resolution,Output_Dir
	//Electrode Potential Values
	//0-31	: Potential values
	private String SimParameters[];
	...
	public DBS_SimA(String args[]){
			//Parse the input arguments into
			//internal storage to use in model	
			if(checkArray(args)){
				//System.arraycopy(args, 0, SimParameters, 0, 40);
				init(true);
			}else{
	...
	private boolean checkArray(String InputArray[]){
		return true;
	}
	
	
	private void init(boolean InputCheck){
		//Execute the model
		//if everything checks out
		if(InputCheck){
			ModelUtil.initStandalone(true);
			Model Solution = run();
			Solution.sol("sol1").runAll();
	...
	private Model run() {
		...
		model.name("32ElectrodeModelRefined_API.mph");
		...
		.set("filename", "C:\\Users\\A\\Dropbox\\DBS_Project\\Code\\AranCode\\DBS_Data_Analysis\\data_export160314.csv");
